...
function card(imgURL, header, paragraph, navigationURL) {
    ...
    cardsContent.classList.add("card-content");

    const prg = document.createElement("p");
    prg.textContent = paragraph;

    const link = document.createElement("a");
    link.href = navigationURL;

    cardsContent.appendChild(prg);
    cardsContent.appendChild(link);

    cards.appendChild(cardsContent);
    ...
    cardDataList.forEach( (item) => {
        const cardComponent = card(item.imgURL, item.header, item.paragraph, item.navigationURL);
        document.querySelector("div.container").appendChild(cardComponent);
    })
